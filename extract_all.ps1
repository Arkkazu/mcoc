...
$outFile = "C:\python\mcoc\champions_all.txt"
...
while ($i -lt $len - 10) {
    ...
    $f2 = ""; $f3 = ""; $f4 = ""; $f5 = ""
    $fc = 0
    while ($j -lt $msgEnd -and $fc -lt 30) {
        $fc++
        if ($j -ge $msgEnd) { break }
        $tb = $bytes[$j]; $j++
        $wt = $tb -band 7; $fn = $tb -shr 3
        if ($wt -eq 2) {
            $fL = 0; $fs = 0
            do {
                if ($j -ge $msgEnd) { break }
            ...
            $j += $fL
        } elseif ($wt -eq 0) { do { if ($j -ge $msgEnd) { break }; $b = $bytes[$j]; $j++ } while ($b -band 0x80)
        } elseif ($wt -eq 5) { $j += 4
        } elseif ($wt -eq 1) { $j += 8
        } else { break }
    }
    $entries.Add([PSCustomObject]@{ Id=$id; Type=$f2; Condition=$f3; Trigger=$f4; Filter=$f5 })
    $i = $msgEnd
}

Write-Host "エントリー数: $($entries.Count)"
$typeOk = ($entries | Where-Object { $_.Type -ne "" }).Count
Write-Host "Typeあり: $typeOk"
...
foreach ($grp in $groups) {
    ...
    $sw.WriteLine($s80d)
    $hdr = "  " + "アビリティID".PadRight(55) + "効果タイプ".PadRight(28) + "トリガー"
    $sw.WriteLine($hdr)
    $sw.WriteLine($s80d)
    foreach ($e in ($grp.Group | Sort-Object { $_.Id })) {
        $line = "  " + $e.Id.PadRight(55) + $e.Type.PadRight(28) + $e.Trigger
        $sw.WriteLine($line)
...
$fi = Get-Item $outFile
Write-Host "完了: $outFile ($([math]::Round($fi.Length/1MB,2)) MB / $($fi.Length) bytes)"
Remove-Item "C:\python\mcoc\_entries.xml" -ErrorAction SilentlyContinue
